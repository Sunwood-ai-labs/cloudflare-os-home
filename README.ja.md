...
続編X投稿用に、tailnet限定の[4枚＋4枚シミュレーター](https://<tailnet-host>:8891/)も用意しました。メイン投稿に4枚、直列返信に4枚、最後にGitHub URLだけの別返信を置く構成です。元の実スクショはリポジトリに残し、非公開の接続先URLを含む2枚だけ公開用コピーでマスクしています。
